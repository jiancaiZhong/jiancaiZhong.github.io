...
> - JDK8 之后，Oracle-JDK 就只支持商用了
> - JDK 开源替代品，如 `OpenJDK`,`Zulu`,[下载地址汇总](https://www.injdk.cn/)
...
直接去[官网](https://tomcat.apache.org/)下载 zip 包，配置好 Java 环境后就能直接启动了
...
>
> 因为系统会收集运行状态如流量，CPU，键盘录入等信息，来产生的熵去生成真正的随机数。由于大部分的云主机熵值都不够，导致启动时卡在这里。可以用 urandom(伪随机) 来替代 random 来生成随机数即可。
>
> 解决方法很简单，修改 Tomcat 的的启动参数，指定使用 urandom 或者修改 jdk 的配置指定使用 urandom
...
```bash
systemctl [status|start|stop|restart|enable|disable] tomcat.service
```
...
> **说明**
>
> - 图方便可以直接把开机启动后要执行的命令可以追加在 `rc.local`后面
> - 位置：`/etc/rc.d/rc.local`
> - 给予权限：` chmod 777 /etc/rc.d/rc.local`
...
配置文件位于 `/etc/php.ini`和 `/etc/php-fpm.d/www.conf`,电脑内存小的话建议调整下 php-fpm
...
> - centos7 默认自带 python2.x，同时 yum 依赖 2.x 版本的
> - 注意在 `/usr/bin/`目录下共有 6 个 python 相关命令
> - 更多请参考 [CentOS安装Python（以Python3.8为例）](https://jiancai.life/posts/2022/10/24/CentOS%E5%AE%89%E8%A3%85Python-%E4%BB%A5Python3.8%E4%B8%BA%E4%BE%8B.html)
...
> CentOS 自带的 git版本很古老，一些自动化工具需要 2.x 以上的版本，升级方法主要有：重新编译安装或者添加其他源进行安装
